...
import { Car, Upload, Trash2, Images, ChevronLeft, ChevronRight, Plus } from 'lucide-react'

const STATUSES = ['IMPORTED', 'IN_TRANSIT', 'ARRIVED', 'AVAILABLE', 'SOLD']
const emptyForm = { vin: '', make: '', model: '', year: '', color: '', price: '', status: 'IMPORTED', description: '', engineType: '', transmission: '', mileage: '' }
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow group">
      ...
      <div className="relative h-44 bg-gray-100 overflow-hidden">
        {mainImg ? (
          <img src={mainImg} alt={`${v.make} ${v.model}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
        ) : (
      ...
      <div className="p-4">
        ...
        <div className="flex gap-1.5 mt-3">
          <button onClick={onDetail} className="flex-1 text-xs bg-blue-50 hover:bg-blue-100 text-blue-600 py-1.5 rounded-lg transition-colors flex items-center justify-center gap-1">
            <Images size={12} /> Details
          </button>
          <button onClick={onEdit} className="flex-1 text-xs bg-gray-50 hover:bg-gray-100 text-gray-600 py-1.5 rounded-lg transition-colors">Edit</button>
          <button onClick={onStatus} className="flex-1 text-xs bg-purple-50 hover:bg-purple-100 text-purple-600 py-1.5 rounded-lg transition-colors">Status</button>
        </div>
      </div>
    </div>
  )
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="lg:w-1/2">
          <div className="relative h-64 bg-gray-100 rounded-xl overflow-hidden mb-3">
            ...
            {images[activeImg] && (
              <button
                onClick={() => handleDelete(images[activeImg].id)}
          ...
          <div className="mt-3">
            ...
            <button
              ...
              disabled={uploading}
              className="w-full border-2 border-dashed border-gray-200 hover:border-blue-400 text-gray-500 hover:text-blue-500 rounded-xl py-3 text-sm transition-colors flex items-center justify-center gap-2 disabled:opacity-60"
            >
              <Upload size={16} />
        ...
        <div className="lg:w-1/2 space-y-3">
          <div className="flex items-center gap-2">
            <Badge status={v.status} />
            <span className="text-gray-400 text-xs font-mono">{v.vin}</span>
          </div>
...
function VehicleForm({ form, setForm, onSubmit, error, showVin, showStatus }) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-3">
      {showVin && <Field label="VIN *"><input {...f('vin')} required className={input} /></Field>}
      <div className="grid grid-cols-2 gap-3">
